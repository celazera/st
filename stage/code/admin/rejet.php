...
// Query to fetch rejected offers
$sql = "SELECT o.idOffre, o.titre AS offre_titre, o.pdfFile, a.company_name, b.titre AS besoin_titre
        ...
        WHERE o.etat = 'refusée'";

$result = $conn->query($sql);

// Initialize array to store rejected offers
$rejected_offres = [];
...
if ($result->num_rows > 0) {
    while($row = $result->fetch_assoc()) {
        ...
        // Create a list item for each rejected offer
        $rejected_offres[] = "<tr>
                                    <td>$company_name</td>
                                    <td>$offre_titre</td>
...
<body>
    ...
    <div class="content">
        <h2>Liste des Offres Refusées</h2>
        <table>
            <thead>
                <tr>
                    <th>Entreprise</th>
            ...
            <tbody>
                <?php
                if (!empty($rejected_offres)) {
                    foreach ($rejected_offres as $offre) {
                        echo $offre;
                    }
                } else {
                    echo "<tr><td colspan='5'>Aucune offre refusée trouvée</td></tr>";
                }
                ?>
            </tbody>
        </table>
